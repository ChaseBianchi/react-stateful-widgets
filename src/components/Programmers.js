...
export default function Programmers() {
  ...
  const [array, setArray] = useState(listOfAwesome)
  ...
  const getNameOfFeatured = () => {
    ...
    for(let i = 0; i<array.length;i++){
      if(array[i].id===person){return array[i].name}
    }
  };
  ...
  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: person===null ? 'royalblue' : 'gold', // 🤔 color turns to gold, when celebrating
  };
      ...
      <div className='programmers'>
        {
          ...
          listOfAwesome.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => {setPerson(dev.id)}}>Feature</button>
            </div>
          )
        }
      </div>
